...
jQuery.Class('ModuleBuilder_LayoutEditor_Js', {
}, {
	...
	registerRelatedListEvent : function(){
        var thisInstance = this;
		contents 		 = jQuery('.relatedlists');
		contents.find('.relcheck').change(function(e) {
	...
	showFieldEditModel: function (data, blockId, fieldContainer) {
		...
		//data.find('input[name="isquickcreatesupported"]').val('zddfd')
		...
		var formFieldname = form.find('select[name="fieldType"]').val();
		...
		if ((formFieldname != 'Picklist') && (formFieldname != 'MultiSelectCombo')) {
	...
	registerAddCustomFieldEvent: function () {
		...
		contents.find('.addCustomField').click(function (e, params) {
			...
			var fieldAddabelHolder = addFieldContainer.find('.pull-left');
			fieldAddabelHolder.html(app.vtranslate('JS_LBL_CREATE_CUSTOM_FIELD'));
			var modalParams = {
				cb: function (data) {
...
///
	 ...
	 * Function to register change event for fieldType while adding custom field
	 */
	registerFieldTypeChangeEventN: function (form, nameAttrsList) {},
	/**
	...
	setFieldDetails: function (result, form) {
		//var fieldId = form.find('.fieldProperties').attr('data-field-id');
		
		var fieldlabelHolder = form.find('.fieldLabels');
		fieldlabelHolder.append(result['label']);
		
		var fieldTypeLabels = form.find('.fieldTypeLabel');
		fieldTypeLabels.append(result['tksfieldtype']);
		var MandatorySymbol = fieldlabelHolder.find('.redColor');
		var mTitle, meTitle, qTitle, sTitle, hTitle;

		if (result['mandatory']=='false' && result['mandatory']!='') {
		...
		if (result['quickcreate']=='true' && result['mandatory']=='true') {
			if (result['mandatory'])
	...
	displayNewCustomBlock: function (result) {
		var thisInstance 	= this;
		var contents 		= jQuery('#detailViewLayout').find('.editViewBody');
		var beforeBlockId 	= result['beforeBlockId'];

		if( beforeBlockId == 0)
		{
			var beforeBlock = jQuery('#moduleBlocks').find('.tksfirstblock');
			beforeBlockId = 1;
		}
		else
		{
			var beforeBlock = contents.find('.block_'+beforeBlockId);
			beforeBlockId 	= parseInt( beforeBlockId ) + parseInt(1);
		}
		...
		newBlockCloneCopy.find('#tksblockid').val(result['id']);
		newBlockCloneCopy.find('#tksblocklabel').val(result['label']); 	
	...
	registerDeleteCustomBlockEvent: function () {
		...
		contents.on('click', '.deleteCustomBlock', function(e) {
			...
			/*if(thisInstance.registerSequenceFirst() == false)
			{
				return false;
			}	*/											  
			var currentTarget 	= jQuery(e.currentTarget);
	...
	registerDeleteCustomFieldEvent: function (contents) {
		...
		contents.find('a.deleteCustomField').click(function (e) {
			...
			var blockId = currentTarget.closest('.editFieldsTable').data('blockId');
			var message = app.vtranslate('JS_LBL_ARE_YOU_SURE_YOU_WANT_TO_DELETE');
			...
			app.helper.showConfirmationBox({'title': app.vtranslate('LBL_WARNING'),
				...
				buttons		:{
						...
						thisInstance.deleteCustomField(fieldId,blockId).then(
							function (data) {
	 ...
	 */
	deleteCustomField: function (fieldId,blockId) {
		var thisInstance = this;
		var aDeferred = jQuery.Deferred();
		app.helper.showProgress();
		var params = {};
	...
	showRelatedTabModulesList: function (relatedContainer, extraParams) {
		...
		//jQuery('#relatedTabOrder').css("display", "block");
		jQuery('#editViewBody').css("display", "none");

		var thisInstance = this;
		var params = {};
		params['module'] 		= app.getModuleName();
		params['parent'] 		= app.getParentModuleName();
		params['sourceModule'] 	= jQuery('#tks_modulename').val();
		params['view'] 			= 'Index';
		...
		AppConnector.request(params).then(
			function(data) {
				...
				jQuery('#relatedTabOrder').css("display", "block");
				relatedContainer.html(data.result);
	...
	registerEditFieldDetailsClick: function (contents) {
		...
		contents.on('click', '.editFieldDetails', function (e) {
			...
			var addFieldContainers = contents.find('.createFieldModal').clone(true, true);
			addFieldContainers.removeClass('hide');
			var form = addFieldContainers.find('.createCustomFieldForm');
			...
			app.request.get({'data': params}).then(
				function (err, data) {
					...
					if (err === null) {
						if(data['id']==fieldId){
						var fieldEditlabelHolder = addFieldContainers.find('.pull-left');
						fieldEditlabelHolder.html(app.vtranslate('JS_LBL_EDIT_FIELD')+''+data['label']);
						form.find('input[name="fieldLabel"]').val(data['label']);
						form.find('input[name="fieldid"]').val(data['id']);
						form.find('input[name="fieldDefaultValue"]').val(data['defaultvalue']);
						...
						form.find('[name="quickcreate"]').filter(':checkbox').attr('checked', true);
						 ...
						 if(data['quickcreate'] == 'true') {
						form.find('[name="quickcreate"]').filter(':checkbox').attr('checked', true);
						 }
						 if(data['mandatory'] == 'true' && data['quickcreate'] == 'true') {
						 form.find('input[type="checkbox"][name="quickcreate"]')
						.prop('checked', true).attr('readonly', 'readonly');
						 }
						
						 if(data['SummaryField'] == 'true') {
						 form.find('[name="summaryfield"]').filter(':checkbox').attr('checked', true);
						 }
						  if(data['headerfield'] == 'true') {
						 form.find('[name="headerfield"]').filter(':checkbox').attr('checked', true);
						 }
						  if(data['masseditable'] == 'true') {
						 form.find('[name="masseditable"]').filter(':checkbox').attr('checked', true);
						 }
						  form.find('.lengthsupported').remove();
						 
						  form.find('select[name^="fieldType"] option[value="'+data['tksfieldtype']+'"]').attr("selected","selected");
						  form.find('[name="fieldType"]').attr("disabled","disabled");
						  form.find('[name="fieldLabel"]').attr("disabled","disabled");
						  if(data['tksfieldtype']== 'Decimal'){
							  form.find('.decimalsupported').remove();
						  
						  }
						  if(data['tksfieldtype']== 'Picklist'){
							  form.find('.preDefinedValueExists,.picklistOption').remove();
							  form.find('[name="fieldDefaultValue"]').remove();
							 form.find('.defaultValueUi').append('<select name="fieldDefaultValue" class="col-sm-9 select2"></select>');
						for (var i = 0; i < data['picklistvalues'].length; i++) {
						   if(data['picklistvalues'][i] == data['defaultvalue'] ){
							    form.find('[name="fieldDefaultValue"]').append('<option value="'+data['picklistvalues'][i]+'" selected>'+data['picklistvalues'][i]+'</option>');
						   }else{
						   form.find('[name="fieldDefaultValue"]').append('<option value="'+data['picklistvalues'][i]+'">'+data['picklistvalues'][i]+'</option>');
						     }
							}
						  }
						   if(data['tksfieldtype']== 'MultiSelectCombo'){
							 form.find('.preDefinedValueExists').remove();
							 form.find('[name="fieldDefaultValue"]').remove();
							 form.find('.defaultValueUi').append('<select multiple name="fieldDefaultValue" class="col-sm-9 select2"></select>');
							  console.log(data['defaultvalue']);
							 for (var i = 0; i < data['picklistvalues'].length; i++) {
							 if(jQuery.inArray( data['picklistvalues'][i], data['defaultvalue'] ) !== -1){
							    form.find('[name="fieldDefaultValue"]').append('<option value="'+data['picklistvalues'][i]+'" selected>'+data['picklistvalues'][i]+'</option>');
							   }else{
							 	form.find('[name="fieldDefaultValue"]').append('<option value="'+data['picklistvalues'][i]+'">'+data['picklistvalues'][i]+'</option>');
						      }
						...
						app.helper.showModal(addFieldContainers, params);
					}
				});
	...
	registersaveModuleClick: function () {

		var thisInstance = this;
		var contents = jQuery('#saveModule');
		var form = jQuery('#saveModuleData');
	...
	/**
	 * Function to check Save Sequence Button is enable or not and prompt message
	 */
	registerSequenceFirst : function(){
		var thisInstance = this;
		contents 		 = jQuery('.editViewContainer');
		var seqbutton = contents.find('.saveFieldSequence');

		if(seqbutton.hasClass('hide'))
		{
			return true;
		}
		else
		{
			var params = {};
			params['text'] = app.vtranslate('JS_SEQUENCE_FIRST');
	...
	registerEvents: function () {
		...
		thisInstance.registersaveModuleClick();
...
Vtiger.Class('ModuleBuilder_LayoutEditor_Index_Js', {}, {
	init: function () {
		this.addComponents();
	},
